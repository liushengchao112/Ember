...
- next
	- Upgrade Notes
		...
		- The RVOController no longer handles movement as it turned out that was a bad idea.
			...
			for movement. Some settings may need to be tweaked, but hopefully it should not be too hard.
	- New Features And Improvements
		...
		- AIPath no longer uses the forwardLook variable.
		...
		- The RVO system has been cleaned up a lot.
			- Agents will now always avoid walls and obstacles even if that would put them on a collision course with another agent.
				This helps with a previous problem of agents being able to be pushed into walls and obstacles.
			- The RVOController can now be configured to for XZ space or XY space (2D).
			...
			- Agents are now biased slightly towards passing other agents on the right side, this helps resolve some situations
				with a lot of symmetry much faster.
			- All fuzzy and hard to adjust parameters from the RVOSimulator component have been removed.
				It now has only 4 parameters.
	...
	- Changes
		...
		- The AdaptiveSampling algorithm for local avoidance has been removed. Only GradientDescent is used now.
