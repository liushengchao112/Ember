	...
	public partial class Voxelize {
		...
		* removed in order to match the segments and vertices at tile boundaries. */
		...
		public void VoxelizeInput () {
			...
			int maximumVoxelYCoord = (int)(forcedBounds.size.y / cellHeight);
		...
		void DrawLine (int a, int b, int[] indices, int[] verts, Color col) {
			...
			Debug.DrawLine(VoxelToWorld(verts[p1+0], verts[p1+1], verts[p1+2]), VoxelToWorld(verts[p2+0], verts[p2+1], verts[p2+2]), col);
		}
