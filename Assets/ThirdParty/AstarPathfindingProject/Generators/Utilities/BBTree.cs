...
namespace Pathfinding {
	...
	public class BBTree : IAstarPooledObject {
		...
		#region IAstarPooledObject implementation
		...
		void EnsureCapacity (int c) {
			if (arr.Length < c) {
				var narr = new BBTreeBox[Math.Max(c, (int)(arr.Length*2f))];
		...
		[System.Diagnostics.Conditional("ASTARDEBUG")]
		static void DrawDebugNode (MeshNode node, float yoffset, Color color) {
		...
		public NNInfoInternal Query (Vector3 p, NNConstraint constraint) {
			if (count == 0) return new NNInfoInternal(null);

			var nnInfo = new NNInfoInternal();
		...
		void SearchBoxClosestXZ (int boxi, Vector3 p, ref float closestDist, NNConstraint constraint, ref NNInfoInternal nnInfo) {
			BBTreeBox box = arr[boxi];
		...
		void SearchBox (int boxi, Vector3 p, NNConstraint constraint, ref NNInfoInternal nnInfo) {//, int intendentLevel = 0) {
			BBTreeBox box = arr[boxi];
			...
			if (box.node != null) {
				...
				if (box.node.ContainsPoint((Int3)p)) {
					...
					if (constraint.Suitable(box.node)) {
						if (nnInfo.constrainedNode == null || Mathf.Abs(box.node.position.y - p.y) < Mathf.Abs(nnInfo.constrainedNode.position.y - p.y)) {
							nnInfo.constrainedNode = box.node;
						}
					}
				}
