...
namespace Pathfinding.Serialization {
	...
	public class TinyJsonSerializer {
		...
		TinyJsonSerializer () {
			serializers[typeof(float)] = v => output.Append(((float)v).ToString("R"));
			...
			serializers[typeof(Vector2)] = v => output.AppendFormat("{{ \"x\": {0}, \"y\": {1} }}", ((Vector2)v).x.ToString("R"), ((Vector2)v).y.ToString("R"));
			serializers[typeof(Vector3)] = v => output.AppendFormat("{{ \"x\": {0}, \"y\": {1}, \"z\": {2} }}", ((Vector3)v).x.ToString("R"), ((Vector3)v).y.ToString("R"), ((Vector3)v).z.ToString("R"));
	...
	public class TinyJsonDeserializer {
		...
		System.Object Deserialize (Type tp, System.Object populate = null) {
			...
			if (tpInfo.IsEnum) {
				...
				var res = new Vector2();
				EatField();
				res.x = float.Parse(EatField());
				EatField();
				res.y = float.Parse(EatField());
				Eat("}");
				return res;
