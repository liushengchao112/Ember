...
namespace Pathfinding.RVO.Sampled {
	...
	public class Agent : IAgent {
		...
		internal float radius, height, desiredSpeed, maxSpeed, neighbourDist, agentTimeHorizon, obstacleTimeHorizon;
		...
		/** \copydoc Pathfinding::RVO::IAgent::NeighbourDist */
		...
		/** \copydoc Pathfinding::RVO::IAgent::MovementMode */
		...
		internal void CalculateVelocity (Pathfinding.RVO.Simulator.WorkerContext context) {
			...
			if (!insideAnyVO) {
				...
				if (DebugDraw) DrawCross(calculatedTargetPoint);
				return;
			}
			...
			if (DebugDraw) DrawCross(result+position);
		...
		void GenerateObstacleVOs (VOBuffer vos) {
			...
			for (int i = 0; i < simulator.obstacles.Count; i++) {
				...
				do {
					...
					var p1 = To2D(vertex.position);
					var p2 = To2D(vertex.next.position);

					// Signed distance from the line (not segment, lines are infinite)
					// TODO: Can be optimized
					float dist = VO.SignedDistanceFromLine(p1, vertex.dir, position);

					if (dist >= -0.01f && dist < neighbourDist) {
						...
						var segmentY = Mathf.Lerp(vertex.position.y, vertex.next.position.y, factorAlongSegment);
						...
						if (sqrDistToSegment < neighbourDist*neighbourDist && elevationCoordinate <= segmentY + vertex.height && elevationCoordinate+height >= segmentY) {
		...
		void GenerateNeighbourAgentVOs (VOBuffer vos) {
			...
			for (int o = 0; o < neighbours.Count; o++) {
				...
				// We assume that the other agent will continue to move with roughly the same velocity
				Vector2 otherOptimalVelocity = other.currentVelocity;
		...
		Vector2 GradientDescent (VOBuffer vos, Vector2 sampleAround1, Vector2 sampleAround2) {
			...
			if (DebugDraw) DrawCross(minima1 + position, Color.yellow, 0.5f);
			...
			if (DebugDraw) DrawCross(minima2 + position, Color.magenta, 0.5f);
		...
		Vector2 Trace (VOBuffer vos, Vector2 p, out float score) {
			...
			for (int s = 0; s < MaxIterations; s++) {
				...
				if (DebugDraw) Debug.DrawLine(To3D(prev + position), To3D(p + position), Rainbow(s*0.1f) * new Color(1, 1, 1, 1f));
			}
