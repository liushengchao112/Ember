...
namespace Pathfinding.RVO {
	...
	public interface IAgent {
		 ...
		 * In 3D this will most likely be set to the y coordinate, but in 2D (top down) it should most likely be set to 0 since
		...
		/** Debug drawing.
		 *
		 * \note Assumes that the #Position is in XZ space and the ElevationCoordinate is the Y coordinate.
		 *
		 * \warning This will only work if multithreading on the simulator component has been disabled
		 ...
		 * \note All agents and obstacles need to have this value set to the same value
		 */
		...
		* \see DoubleBuffering */
		...
		public void UpdateObstacle (ObstacleVertex obstacle, Vector3[] vertices, Matrix4x4 matrix) {
			...
			do {
				...
				c.position = identity ? vertices[count] : matrix.MultiplyPoint3x4(vertices[count]);
				c = c.next;
				count++;
			} while (c != obstacle && c != null);
		...
		void PreCalculation () {
			for (int i = 0; i < agents.Count; i++) {
				...
				if (agents[i].MovementMode != agents[0].MovementMode) {
					Debug.LogError("RVO movement modes are inconsistent. Some are set to " + agents[i].MovementMode + " and some to " + agents[0].MovementMode + ". Local avoidance will not work properly while this is the case.");
					return;
				}
			}
		}
