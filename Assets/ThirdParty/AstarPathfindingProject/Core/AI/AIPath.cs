 ...
 * \note The image is slightly outdated, replace forwardLook with pickNextWaypointDist in the image.
 *
...
public class AIPath : MonoBehaviour {
	...
	/** Rotation speed.
	 * Rotation is calculated using Quaternion.SLerp. This variable represents the damping, the higher it is, the faster the character will be able to rotate.
	 */
	...
	public virtual void Update () {
		...
		if (rvoController != null) {
			...
			// TODO: Use CalculateMovementDelta instead for slightly
			// better behaviour at the end of the path (less oscillations)
			dir = rvoController.velocity;
		}
	...
	protected Vector3 CalculateVelocity (Vector3 currentPosition) {
		...
		while (currentWaypointIndex < vPath.Count-1) {
			// There is a "next path segment"
			float dist = VectorMath.SqrDistanceXZ(vPath[currentWaypointIndex], currentPosition);
		...
		// Current path segment goes from vPath[currentWaypointIndex-1] to vPath[currentWaypointIndex]
		// We want to find the point on that segment that is 'pickNextWaypointDist' from our current position.
		// This can be visualized as finding the intersection of a circle with radius 'pickNextWaypointDist'
		// with that segment.
		var p1 = vPath[currentWaypointIndex-1];
		var p2 = vPath[currentWaypointIndex];

		// Calculate the intersection with the circle. This involves some math.
		var t = VectorMath.LineCircleIntersectionFactor(To2D(currentPosition), To2D(p1), To2D(p2), pickNextWaypointDist);
		// Clamp to a point on the segment
		t = Mathf.Clamp01(t);
		Vector3 targetPosition = Vector3.Lerp(p1, p2, t);
		...
		float slowdown = slowdownDistance > 0 ? Mathf.Clamp01(targetDist / slowdownDistance) : 1;
		...
		if (Time.deltaTime > 0) {
			// Make sure we don't overshoot and move too far
