...
namespace Pathfinding {
	...
	public class AstarPathEditor : Editor {
		...
		public override void OnInspectorGUI () {
			...
			if (!stylesLoaded) {
				if (!LoadStyles()) {
					EditorGUILayout.HelpBox("The GUISkin 'AstarEditorSkin.guiskin' in the folder "+editorAssets+"/ was not found or some custom styles in it does not exist.\n"+
		...
		static bool LoadStyles () {
			...
			var skinPath = editorAssets + "/AstarEditorSkin" + (EditorGUIUtility.isProSkin ? "Dark" : "Light") + ".guiskin";
		...
		void DrawGraphInfoArea (GraphEditor graphEditor) {
			...
			if (graphEditor.infoFadeArea.BeginFade()) {
				...
				if (!graphNodeCounts.TryGetValue(graphEditor.target, out pair) || (Time.realtimeSinceStartup-pair.Key) > 2) {
					...
					graphEditor.target.GetNodes(counter);
